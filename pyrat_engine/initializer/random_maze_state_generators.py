...
class RandomMazeStateGenerators:
    ...
    def _simple_random_cheese_state_generator(maze_config: MazeConfig):
        ...
        for i in range(0, cheese_number // 2):
            ...
            previous_cheeses_positions.add(
                (maze_config.width - cheese_x, maze_config.height - cheese_y)
            )
    ...
    def generate_simple_random_cheese_state(maze_config: MazeConfig) -> MazeConfig:
        return MazeConfig.copy(
            cheeses=[
                cheese_position
                for cheese_position in RandomMazeStateGenerators._simple_random_cheese_state_generator(
                    maze_config
                )
            ]
        )
